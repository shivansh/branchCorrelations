...
class SimpleInstrumentation : public SgSimpleProcessing {
   public:
    ...
    void insertCalls(SgIfStmt *astNode, int arr_index, SgBasicBlock *func_body1,
                     SgBasicBlock *func_body2);
    int counter;
};

void SimpleInstrumentation::insertCalls(SgIfStmt *astNode, int arr_index,
                                        SgBasicBlock *func_body1,
                                        SgBasicBlock *func_body2) {
    ...
    SgExprStatement *callStmt_1 = buildFunctionCallStmt(
        name1, buildVoidType(), buildExprListExp(buildIntVal(arr_index)),
        astNode);
    SgExprStatement *callStmt_2 = buildFunctionCallStmt(
        name2, buildVoidType(), buildExprListExp(buildIntVal(arr_index)),
        astNode);
...
void SimpleInstrumentation::visit(SgNode *astNode) {
    ...
    if (globalScope != NULL) {
        ...
        counter = 0;
        for (Rose_STL_Container<SgNode *>::iterator i = ifStmtList.begin();
             i != ifStmtList.end(); i++) {
        ...
        SgInitializedName *var1_init_name =
            buildInitializedName("index", ref_type);
        ...
        SgFunctionDeclaration *func1 = buildDefiningFunctionDeclaration(
            func_name1, buildVoidType(), parameterList, globalScope);
        ...
        SgExprStatement *new_stmt1 = buildExprStatement(
            buildPlusPlusOp(buildPntrArrRefExp(lhs_var_ref1, rhs_var_ref1)));

        // Create a defining functionDeclaration (with a function body)
        SgName func_name2 = "increment_else";
        SgFunctionDeclaration *func2 = buildDefiningFunctionDeclaration(
            func_name2, buildVoidType(), parameterList, globalScope);
        ...
        SgExprStatement *new_stmt2 = buildExprStatement(
            buildPlusPlusOp(buildPntrArrRefExp(lhs_var_ref2, rhs_var_ref2)));
        ...
        SgFunctionDeclaration *func = buildDefiningFunctionDeclaration(
            evaluate_func, buildVoidType(), buildFunctionParameterList(),
            globalScope);
        ...
        SgExprStatement *while_condition = buildExprStatement(
            buildLessThanOp(while_lhs_ref, buildIntVal(counter)));
        SgWhileStmt *while_stmt =
            buildWhileStmt(while_condition, while_body, NULL);
        SgVariableDeclaration *i_var =
            buildVariableDeclaration(i_name, buildIntType());

        prependStatement(while_stmt, func_body);
        SgExprStatement *print_func = buildFunctionCallStmt(
            "printf", buildIntType(),
            buildExprListExp(buildStringVal("%d %d\\n"),
                             buildPntrArrRefExp(lhs_var_ref1, while_lhs_ref),
                             buildPntrArrRefExp(lhs_var_ref2, while_lhs_ref)),
            func_body);
        insertStatementBefore(while_body, print_func, true);
        prependStatement(i_var, func_body);
        SgExprStatement *assign_i = buildAssignStatement(
            buildVarRefExp(i_name, func_body), buildIntVal(0));
        ...
        for (iter = funcDefList.begin(); iter != funcDefList.end(); iter++) {
            ...
            SgExprStatement *callStmt1 = buildFunctionCallStmt(
                "evaluate", buildIntType(), buildExprListExp(), body);
